...
export const Cover = ({ children, className }: { children?: React.ReactNode; className?: string }) => {
  ...
  const [beamPositions, setBeamPositions] = useState<{ top: number; duration: number; delay: number }[]>([])
  ...
  return (
    <div
      ...
      ref={ref}
      className='group/cover relative inline-block w-full rounded-sm px-2 py-2 transition duration-200'
    >
      <AnimatePresence>
        {hovered && (
          <motion.div
...
export const Beam = ({
  ...
  const id = useId()
  const [gradientDelays, setGradientDelays] = useState({ delay: 0, repeatDelay: delay ?? 1 })
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      if (hovered) {
        ...
        setGradientDelays({ delay: 0, repeatDelay: delay ?? 1 })
      }
    }, 0)
      ...
      <defs>
        <motion.linearGradient
          ...
          transition={{
            ...
            ease: 'linear',
            repeat: Infinity,
            delay: gradientDelays.delay,
            repeatDelay: gradientDelays.repeatDelay
          }}
        >
